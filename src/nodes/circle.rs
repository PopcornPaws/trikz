...
impl AnchorT for Circle {
    fn anchor(&self, anchor: Anchor) -> Vector2 {
        ...
        let (radius, angle) = match anchor {
            ...
            Anchor::North => return self.origin + Vector2::new(0.0, self.radius),
            Anchor::East => return self.origin + Vector2::new(self.radius, 0.0),
            Anchor::South => return self.origin + Vector2::new(0.0, -self.radius),
            Anchor::West => return self.origin + Vector2::new(-self.radius, 0.0),
...
mod test {
    ...
    fn anchors() {
        ...
        assert_eq!(circle.above_right(5.0, 2.0), circle.northeast() + Vector2::new(5.0, 2.0));
        assert_eq!(circle.above_left(2.0, 1.0), circle.northwest() + Vector2::new(-2.0, 1.0));
        assert_eq!(circle.below_left(1.0, 9.0), circle.southwest() + Vector2::new(-1.0, -9.0));
        assert_eq!(circle.below_right(-9.0, 4.0), circle.southeast() + Vector2::new(-9.0, -4.0));

        let anchor = Anchor::Polar { radius: 2.0 * radius, angle: 135.0 };
